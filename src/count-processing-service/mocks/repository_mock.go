...
func (m *MockCountValueRepository) GetAll(ctx context.Context) ([]domain.CountValue, error) {
...
func (m *MockCountValueRepository) Increase(ctx context.Context, itemID string, amount int, source string) (*domain.CountValue, error) {
	return m.IncreaseFunc(ctx, itemID, amount, source)
}

func (m *MockCountValueRepository) Decrease(ctx context.Context, itemID string, amount int, source string) (*domain.CountValue, error) {
	return m.DecreaseFunc(ctx, itemID, amount, source)
}

func (m *MockCountValueRepository) Reset(ctx context.Context, itemID string, source string) (*domain.CountValue, error) {
	return m.ResetFunc(ctx, itemID, source)
}

type MockCountHistoryRepository struct {
	SaveLogFunc    func(ctx context.Context, log domain.CountLog) error
	GetHistoryFunc func(ctx context.Context, itemID string) ([]domain.CountLog, error)
}

func (m *MockCountHistoryRepository) SaveLog(ctx context.Context, log domain.CountLog) error {
	return m.SaveLogFunc(ctx, log)
}

func (m *MockCountHistoryRepository) GetHistory(ctx context.Context, itemID string) ([]domain.CountLog, error) {
